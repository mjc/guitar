...
pub fn delete_branch(repo: &Repository, branch: &str) -> Result<(), git2::Error> {
    ...
    {
        repo.find_reference(branch_ref_name.as_ref()).map_err(to_git2_error)?;
    }

    if repo.head_name().map_err(to_git2_error)?.as_ref() == Some(&branch_ref_name) {
        return Err(Error::from_str("cannot delete the currently checked out branch"));
    }

    repo.committer_or_set_generic_fallback().map_err(to_git2_error)?;
    {
        let branch_ref = repo.find_reference(branch_ref_name.as_ref()).map_err(to_git2_error)?;
        branch_ref.delete().map_err(to_git2_error)?;
    }
...
pub fn rename_branch(repo: &Repository, old_name: &str, new_name: &str) -> Result<(), Error> {
    ...
    if repo.try_find_reference(new_ref_name.as_ref()).map_err(to_git2_error)?.is_some() {
        return Err(Error::from_str("branch name already exists"));
    }
    let target_oid = {
        let mut branch = repo.find_reference(old_ref_name.as_ref()).map_err(to_git2_error)?;
        branch.peel_to_id().map_err(to_git2_error)?.detach()
    };
    let current_head = repo.head_name().map_err(to_git2_error)?;
    let is_current_branch = current_head.as_ref() == Some(&old_ref_name);

    repo.committer_or_set_generic_fallback().map_err(to_git2_error)?;
    let mut edits = vec![
        RefEdit { change: Change::Update { log: ref_log("branch rename"), expected: PreviousValue::MustNotExist, new: Target::Object(target_oid) }, name: new_ref_name.clone(), deref: false },
    ...
    if is_current_branch {
        edits.push(RefEdit {
            change: Change::Update { log: ref_log("branch rename"), expected: PreviousValue::MustExistAndMatch(Target::Symbolic(old_ref_name)), new: Target::Symbolic(new_ref_name) },
            name: head_ref_name(),
            deref: false,
        });
    }
    repo.edit_references(edits).map_err(to_git2_error)?;
